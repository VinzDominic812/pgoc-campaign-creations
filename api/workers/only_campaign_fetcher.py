    ...
    try:
        ...
        with db.session.begin():
            ...
            if campaign_entry:
                ...
                campaign_entry.last_check_message = "Campaigns updated."
                flag_modified(campaign_entry, "campaigns_data")
                db.session.commit()

        append_redis_message2(user_id, ad_account_id, "Campaign updates saved.")
        return f"Fetched and updated selected campaigns for {ad_account_id}."

    except Exception as e:
        error_message = f"Error fetching campaigns for {ad_account_id}: {e}"
